...
namespace GerenciadorCondominios.DAL.Repositorios
{
    public class HistoricoRecursosRepositorio : RepositorioGenerico<HistoricoRecurso>, IHistoricoRecursosRepositorio
    {
        ...
        public async Task<IEnumerable<HistoricoRecurso>> PagarUltimasMovimentacoes()
        {
            try
            {
                return await _contexto.HistoricoRecursos.Include(hr => hr.Mes).OrderByDescending(hr => hr.HistoricoRecursoId)
                    .Take(5).ToListAsync();
            }
            catch (Exception ex)
            {
