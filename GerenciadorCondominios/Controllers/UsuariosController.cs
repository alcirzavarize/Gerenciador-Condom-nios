...
namespace GerenciadorCondominios.Controllers
{
    public class UsuariosController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (usuario != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            ModelState.AddModelError("", "E-mail e/ou senha ivalidos!");
                            return View(model);
                        }
                    }
                }
        ...
        [Authorize(Roles = "Administrador")]
        [HttpPost]
        public async Task<IActionResult> GerenciarUsuario(List<FuncaoUsuarioViewModel> models)
        {
            ...
            {
                ModelState.AddModelError("", "Não foi possivel atualizar funções do usuário");
                TempData["Exclusao"] = $"Não foi possivel atualizar as informações do usuário {usuario.UserName}";
                return View("GerenciadorUsuario", usuarioId);
            }

            resultado = await _usuarioRepositorio.IncluirUsuarioEmFuncoes(usuario,
                models.Where(x => x.IsSelecionado == true).Select(x => x.Nome));
            ...
            TempData["Atualizacao"] = $"As informações do usuário {usuario.UserName} foram atualizadas";
            return RedirectToAction(nameof(Index));
        }
        [Authorize]
        ...
        public async Task<IActionResult> Atualizar(AtualizarViewModel viewModel, IFormFile foto)
        {
            if (ModelState.IsValid)
            {
                ...
                Usuario usuario = await _usuarioRepositorio.PegarPeloId(viewModel.UsuarioId);
